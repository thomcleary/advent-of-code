...
typedef struct Frequency {
  char value;
  Coord *antennas;
  int64_t num_antennas;
  int64_t antennas_size;
} Frequency;
...
typedef struct AntennaMap {
  ...
  int64_t frequencies_size;
} AntennaMap;

typedef struct AntinodeMap {
  int64_t rows, columns;
  char **map;
  int64_t num_antinodes;
} AntinodeMap;
...
Frequency *frequency_new(char value, Coord location) {
  ...
  frequency->antennas =
      malloc(sizeof(*frequency->antennas) * (size_t)frequency->antennas_size);
...
void frequency_add_antenna(Frequency *frequency, Coord location) {
  if (frequency->num_antennas == frequency->antennas_size) {
    ...
    frequency->antennas =
        realloc(frequency->antennas, sizeof(*frequency->antennas) *
                                         (size_t)frequency->antennas_size);
...
Antinodes frequency_antinodes(Frequency *frequency, Coord limit,
  ...
  int64_t antinodes_size = num_pairs * 2;
  Antinodes antinodes = {
      .value = malloc(sizeof(*antinodes.value) * (size_t)antinodes_size),
      antinodes.length = 0};
  assert(antinodes.value != NULL && "malloc failed");

  for (int64_t i = 0; i < frequency->num_antennas; i++) {
    Coord first = frequency->antennas[i];

    for (int64_t j = 0; j < frequency->num_antennas; j++) {
      if (j == i) {
        continue;
      }
      ...
      if (!resonant_harmonics) {
        ...
        while (antinode_is_within_limits(antinode, limit)) {
          if (antinodes.length == antinodes_size) {
            antinodes_size *= 2;
            antinodes.value =
                realloc(antinodes.value,
                        sizeof(*antinodes.value) * (size_t)antinodes_size);
...
void antenna_map_free(AntennaMap *map) {
  for (int64_t i = 0; i < map->num_frequencies; i++) {
...
void antennamap_add_frequency(AntennaMap *map, Frequency *frequency) {
  if (map->num_frequencies == map->frequencies_size) {
    ...
    map->frequencies =
        realloc(map->frequencies,
                sizeof(*map->frequencies) * (size_t)map->frequencies_size);
...
AntennaMap *antenna_map_parse(Txt *txt) {
  int64_t num_rows = txt->num_lines;
  int64_t num_columns = (int64_t)strlen(txt->lines[0]);
  ...
  map->frequencies =
      malloc(sizeof(*map->frequencies) * (size_t)map->frequencies_size);
  assert(map->frequencies != NULL && "malloc failed");

  for (int64_t row = 0; row < num_rows; row++) {
    char *line = txt->lines[row];

    for (int64_t col = 0; col < num_columns; col++) {
      char ch = line[col];
...
void antinode_map_print(AntinodeMap *map) {
  for (int64_t row = 0; row < map->rows; row++) {
    printf("%s\n", map->map[row]);
  }
  printf("\n");
}

AntinodeMap *antinode_map_new(int64_t rows, int64_t columns) {
  ...
  antinode_map->map =
      malloc(sizeof(*antinode_map->map) * (size_t)antinode_map->columns);
  assert(antinode_map->map != NULL);

  for (int64_t i = 0; i < antinode_map->rows; i++) {
    char *row = malloc((size_t)antinode_map->columns + 1);
    assert(row != NULL && "malloc failed");

    memset(row, '.', (size_t)antinode_map->columns);
...
AntinodeMap *antinode_map_generate(AntennaMap *antenna_map,
  ...
  Coord antinode_limit = {.row = antenna_map->rows - 1,
                          .column = antenna_map->columns - 1};

  for (int64_t i = 0; i < antenna_map->num_frequencies; i++) {
    Antinodes antinodes = frequency_antinodes(
        antenna_map->frequencies[i], antinode_limit, resonant_harmonics);

    for (int64_t j = 0; j < antinodes.length; j++) {
...
int main(void) {
  ...
  int64_t num_antinodes = antinode_map->num_antinodes;

  AntinodeMap *antinode_map_with_resonant_harmonics =
      antinode_map_generate(antenna_map, true);
  int64_t num_antinodes_with_resonant_harmonics =
